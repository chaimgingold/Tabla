...
static char * getDeviceDescription(UINT joystickID, JOYCAPS caps) {
	...
	_snprintf_s(subkey, REG_STRING_MAX, REG_STRING_MAX, "%s\\%s\\%s", REGSTR_PATH_JOYCONFIG, caps.szRegKey, REGSTR_KEY_JOYCURR);
	...
	if (result == ERROR_SUCCESS) {
		...
		_snprintf_s(value, REG_STRING_MAX, REG_STRING_MAX, "Joystick%d%s", joystickID + 1, REGSTR_VAL_JOYOEMNAME);
		nameSize = sizeof(name);
		result = RegQueryValueEx(key, value, NULL, NULL, (LPBYTE) name, &nameSize);
		RegCloseKey(key);
		
		if (result == ERROR_SUCCESS) {
			_snprintf_s(subkey, REG_STRING_MAX, REG_STRING_MAX, "%s\\%s", REGSTR_PATH_JOYOEM, name);
    ...
    bufflength = strlen(caps.szPname) + 1;
	description = malloc(bufflength);
	strcpy_s(description, strlen(caps.szPname) + 1, caps.szPname);
